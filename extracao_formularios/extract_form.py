...
def main():
    ...
    url_imagem = "https://webagencia.com.br/formulario.jpeg" 
        
    settings = Settings(
        endpoint= os.getenv("endpoint"),
        api_version="2025-05-01-preview",
        subscription_key=os.getenv("key"),
        aad_token=os.getenv("token"),
        analyzer_id="formulario",
        file_location=url_imagem,
    )
    ...
    for field in result['result']['contents'][0]['fields']:
        print(f"{field[0].upper() + field[1:]}: {result['result']['contents'][0]['fields'][field]['valueString']}")
